...
test('EditableInput renders correctly', function () {
  var tree = renderer
    .create(
      React.createElement(EditableInput, { label: 'Hex', placeholder: '#fff' })
    )
    .toJSON()
  expect(tree).toMatchSnapshot()
})
...
test('Swatch renders correctly', function () {
  var tree = renderer
    .create(
      React.createElement(Swatch, { color: '#333', style: { opacity: '0.4' } })
    )
    .toJSON()
  expect(tree).toMatchSnapshot()
})
...
test('Swatch renders custom title correctly', function () {
  var tree = renderer
    .create(React.createElement(Swatch, { color: '#fff', title: 'white' }))
    .toJSON()
  expect(tree).toMatchSnapshot()
})
